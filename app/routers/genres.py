...
#POST
@router.post('/create',status_code=status.HTTP_201_CREATED,response_model=schemas.GenreOut)
async def create_genre(genre:schemas.Genre,db: Session = Depends(get_db),current_user: int = Depends(oauth2.get_current_user)):
...
async def update_genre(genre_id:int, edit_genre: schemas.Genre,db:Session = Depends(get_db),current_user: int = Depends(oauth2.get_current_user)):
    genre_query = db.query(models.Genre).filter(models.Genre.id == genre_id)
    genre = genre_query.first()
    
    if genre == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"Genre does not exist")
