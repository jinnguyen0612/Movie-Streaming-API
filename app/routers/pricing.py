...
async def create_pricing(pricing:schemas.Pricing,db: Session = Depends(get_db),current_user: int = Depends(oauth2.get_current_user)):
...
async def update_price(pricing_id:int, edit_pricing: schemas.Pricing,db:Session = Depends(get_db),current_user: int = Depends(oauth2.get_current_user)):
    try:
        ...
        if pricing == None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail=f"Pricing does not exist")
...
async def update_pricing_status(pricing_id:int, db:Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    pricing_query = db.query(models.Pricing).filter(models.Pricing.id == pricing_id)
    pricing = pricing_query.first()
    
    if pricing is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Pricing does not exist")
