...
async def create_actor(actor:schemas.Actor,db: Session = Depends(get_db),current_user: int = Depends(oauth2.get_current_user)):
...
async def get_all_actors(db:Session = Depends(get_db),current_user: int = Depends(oauth2.get_current_user)):
    actors = db.query(models.Actor).all()
    return actors

@router.get('/get/{actor_id}', response_model=schemas.Actor)
async def get_actor(actor_id: int, db:Session=Depends(get_db),current_user: int = Depends(oauth2.get_current_user)):
    actor = db.query(models.Actor).filter(models.Actor.id==actor_id).first()
    if not actor:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Actor does not exist")
    return actor
...
def get_film_actors(film_id: int, db: Session = Depends(get_db)):
    ...
    actors = db.query(models.Actor).join(
        models.Film_Actor,
        models.Actor.id == models.Film_Actor.actor_id
    ).filter(models.Film_Actor.film_id == film_id).all()
...
async def updateActor(actor_id:int, editActor: schemas.Actor,db:Session = Depends(get_db),current_user: int = Depends(oauth2.get_current_user)):
    actor_query = db.query(models.Actor).filter(models.Actor.id == actor_id)
    actor = actor_query.first()
    
    if actor == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"Actor does not exist")
