...
def add_payment_for_film(film_id: int, days: int, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
    ...
    payment = models.Payment(user_id=current_user.id, film_id=film_id, pay=pay, status=0, end_date=end_date)
    db.add(payment)
    db.commit()
    db.refresh(payment)
...
def add_payment_for_film(pricing_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
    ...
    payment = models.Payment(user_id=current_user.id, pricing_id=pricing_id, pay=pay, status=0, end_date=end_date)
    db.add(payment)
    db.commit()
    db.refresh(payment)
